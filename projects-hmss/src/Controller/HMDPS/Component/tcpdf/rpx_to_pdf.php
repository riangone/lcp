...
class rpx_to_pdf
{
    ...
    public function to_pdf()
    {
        ...
        $tcpdf_file = str_replace("HMDPS/Component", "Component", $this->file_path) . "/" . 'tcpdf/tcpdf.php';
                    ...
                    if (strpos($rpx_file_name, $key) !== false) {
                        ...
                        if ($this->rpx_file_name == "rptDenpyoinsatu2" || $this->rpx_file_name == "rptShiharaiDenpyo2") {
                            ...
                            if ($this->rpx_file_name == "rptDenpyoinsatu2") {
                                ...
                                $pdf->write1DBarcode($datas[$i]['SYOHY_NO'], 'C39', 129, $pdf->GetY() + 8.5, 75, 20, null, $style, 'N');
            ...
            foreach ($arrVal as $partVal) {
                ...
                while (mb_strlen($remainingText, "UTF-8") > 0 && $rowNumber <= $MultirowNum) {
                    ...
                    for ($i = 1; $i <= $textLength; $i++) {
                        ...
                        $tmp_w = $pdf->GetStringWidth($substr);

                        // 現在の文字が全角スペースである場合 +0.01
                        $currentChar = mb_substr($remainingText, $i - 1, 1, "UTF-8");
                        if ($currentChar === '　') {
                            $widthAdjustment += 0.01;
                        }

                        $tmp_w += $widthAdjustment;
                        if ($tmp_w >= $w) {
