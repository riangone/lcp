...
use App\Model\R4\R4K\FrmSyasyuUriageList;

class FrmSyasyuUriageListController extends AppController
{
    public $autoLayout = TRUE;
    // public $autoRender = false;
    public $FrmSyasyuUriageList = "";
    ...
    {
        $this->render('index', 'FrmSyasyuUriageList_layout');
    }
    ...
    public function frmKanrSyukeiLoad()
    {
        $result = array();
        try {
            $this->FrmSyasyuUriageList = new FrmSyasyuUriageList();
            $result = $this->FrmSyasyuUriageList->fncSelect();
            if (!$result['result']) {
    ...
    public function fncPrintTougetu()
    {
        $result = array();
        $cboYMTo = "";
        try {
            $cboYMTo = $_POST['data'];
            $this->FrmSyasyuUriageList = new FrmSyasyuUriageList();
            $result = $this->FrmSyasyuUriageList->fncPrintTougetu($cboYMTo);
            if (!$result['result']) {
                throw new \Exception($result['data']);
            }
            if (count($result['data']) > 0) {
                ...
                include_once $path_rpxTopdf . '/Component/tcpdf/rptSyasyuUriageList.inc';
                ...
                $tmp["data"] = $tmp_data;
                $tmp["mode"] = "5";
                $datas["rptSyasyuUriageList"] = $tmp;
                $rpx_file_names["rptSyasyuUriageList"] = $data;
    ...
    public function fncPrintTouki()
    {
        ...
        try {
            ...
            $this->FrmSyasyuUriageList = new FrmSyasyuUriageList();
            $result = $this->FrmSyasyuUriageList->fncPrintTouki($cboYMFrom, $cboYMTo);
            if (!$result['result']) {
                throw new \Exception($result['data']);
            }
            if (count($result['data']) > 0) {
    ...
    public function fncPrintDouble()
    {
        ...
        try {
            ...
            $cboYMTo = $_POST['data']['cboYMTo'];
            $this->FrmSyasyuUriageList = new FrmSyasyuUriageList();
            $result1 = $this->FrmSyasyuUriageList->fncPrintTougetu($cboYMTo);
            if (!$result1['result']) {
                throw new \Exception($result1['data']);
            }
            $result['data'] = $result1['data'];
            $result2 = $this->FrmSyasyuUriageList->fncPrintTouki($cboYMFrom, $cboYMTo);
            if (!$result2['result']) {
            ...
            if (count($result1['data']) > 0 || count($result2['data']) > 0) {
                ...
                if (count($result1['data']) > 0) {
                    ...
                    $tmp1["data"] = $tmp_data;
                    $tmp1["mode"] = "5";
                    $datas["rptSyasyuUriageList"] = $tmp1;
                    $rpx_file_names["rptSyasyuUriageList"] = $data;
                }
                if (count($result2['data']) > 0) {
                    ...
                    $tmp2["data"] = $tmp_data;
                    $tmp2["mode"] = "5";
                    $datas["rptSyasyuUriage1"] = $tmp2;
                    $rpx_file_names["rptSyasyuUriage1"] = $data;
                }
                $obj = new \rpx_to_pdf($rpx_file_names, $datas);
                $pdfPath = $obj->to_pdf2();
                $result['pdfmark'] = TRUE;
