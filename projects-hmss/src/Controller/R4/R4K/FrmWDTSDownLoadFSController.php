 ...
 * 20151111           #2265						   BUG                              li
 * --------------------------------------------------------------------------------------------
 */
namespace App\Controller\R4\R4K;

use App\Controller\AppController;
use App\Model\R4\R4K\FrmWDTSDownLoadFS;

class FrmWDTSDownLoadFSController extends AppController
{
    public $autoLayout = TRUE;
    // public $autoRender = false;
    public $const_dl_kind = "1";
    public $const_dl_step = "1";
    // public $ClsComFnc = '';
    ...
    {
        $this->render('index', 'FrmWDTSDownLoadFS_layout');
    }
...
//				}
            ...
            $this->FrmWDTSDownLoadFS = new FrmWDTSDownLoadFS();

            //-----ダウンロード対象グループを取得する----
            //$result = $this -> FrmWDTSDownLoadFS -> fncGetDate($this -> strDBLink);
            $result = $this->FrmWDTSDownLoadFS->fncGetTableID($this->strDBLink, $this->const_dl_kind);
            if (!$result['result']) {
            ...
            //コンピュータ名取得
            $this->strClientNM = $this->FrmWDTSDownLoadFS->fncGetClient();
            ...
            //トランザクション処理開始
            $result = $this->FrmWDTSDownLoadFS->Do_conn();
            ...
            $this->FrmWDTSDownLoadFS->Do_transaction();
            $this->blnTran = TRUE;

            //HFTS_TARNSFER_LISTにINSERTする
            $fncInsHFTS_TRANSFER_LISTSQL = $this->ClsFileObserver->fncInsHFTSTRANSFERLIST("003", $this->strStartDate, $this->strStartTime, $this->strClientNM, "R4連携集計システム用ダウンロード", "", "", $this->strDBLink);
            $result = $this->FrmWDTSDownLoadFS->fncInsHFTSTRANSFERLIST($fncInsHFTS_TRANSFER_LISTSQL);
            ...
            //コミット
            $this->FrmWDTSDownLoadFS->Do_commit();
            ...
            //ﾌｧｲﾙ名を設定
            $strFileNM = $strFileNM . "003" . $this->strStartDate . $this->strStartTime . $this->strClientNM . ".txt";
            //インスタンス作成
    ...
    public function finally()
    {
        if ($this->blnTran) {
            //ロールバック
            $this->FrmWDTSDownLoadFS->Do_rollback();
        } else {
            //ｴﾗｰが発生した場合、ファイル転送ﾘｽﾄにｴﾗｰ内容を更新する
            if ($this->blnFTSUpd == TRUE && $this->strMessage != "") {
                $clsFos = $this->ClsFileObserver->fncUpdTrnTbl("003", $this->strStartDate, $this->strStartTime, $this->strClientNM, 0, $this->ClsFileObserver->enmState["ErrIrregular"], $this->strMessage, TRUE, $this->strDBLink);
        ...
        if ($this->blnDBconectFlg) {
            $this->FrmWDTSDownLoadFS->Do_close();
        }
    }
