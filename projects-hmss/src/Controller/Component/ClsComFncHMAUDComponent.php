...
use App\Model\HMAUD\Component\ClsComFncHMAUD;
use App\Model\R4\Component\ClsComFnc;
use stdClass;

class ClsComFncHMAUDComponent extends Component
{
    //メンバー変数宣言
    public $xml = "";
    public $xml_path = "";
    ...
    {
        try {
            $this->ClsComFnc = new ClsComFncHMAUD();
    ...
    public function FncGetBusyoMstValue()
    {
        ...
        try {
            //** 初期設定
            $this->ClsComFncHMAUD = new ClsComFncHMAUD();
            $Do_Execute = $this->ClsComFncHMAUD->FncGetBusyoMstValue();
            if (!$Do_Execute['result']) {
                throw new \Exception($Do_Execute['data']);
            }
    ...
    public function halfToFull($str)
    {
        $fullStr = "";
        $strTemp = "";
        for ($i = 0; $i < mb_strlen($str, "UTF-8"); $i++) {
            //複合仮名（三つ文字ギャ）半角から全角転換
            $strTemp = $this->toThreeChar(substr($str, $i, 3));
            if ($strTemp != "") {
                $fullStr = $fullStr . $strTemp;
                $i += 2;
            } else {
                //複合仮名（二文字キャ）半角から全角転換
                $strTemp = $this->toTwoChar(substr($str, $i, 2));
                if ($strTemp != "") {
                    $fullStr = $fullStr . $strTemp;
                    $i += 1;
                } else {
                    //仮名（二文字ガ）半角から全角転換
                    $strTemp = $this->toOneChar(substr($str, $i, 2));
                    if ($strTemp != "") {
                        $fullStr = $fullStr . $strTemp;
                        $i += 1;
                    } else {
                        //普通仮名（1文字ア）半角から全角転換
                        $strTemp = $this->toChar(substr($str, $i, 1));
                        if ($strTemp != "") {
                            $fullStr = $fullStr . $strTemp;
                        } else {
                            //横棒(－)から縦棒(｜)転換
                            $strTemp = $this->toLine(substr($str, $i, 1));
                            if ($strTemp != "") {
                                $fullStr = $fullStr . $strTemp;
                            } else {
                                //仮名以外1文字
                                $strTemp = trim(substr($str, $i, 1));
                                if ($strTemp != "") {
                                    $fullStr = $fullStr . substr($str, $i, 1) . "<br />";
                                }
                            }
                        }
                    }
    ...
    //複合仮名（三つ文字）半角から全角転換
    public function toThreeChar($str)
    {
        $strTempFull = "";
        $strTempHalf = "";
        $count = 0;
        ...
        for ($i = 0; $i < mb_strlen($strTempHalf, "UTF-8"); $i += 3) {
            $str2 = "";
            $count += 1;
            $str2 = substr($strTempHalf, $i, 3);
            if ($str == $str2) {
                $strTemp = substr($strTempFull, $count * 2 - 2, 2);
                $chrTemp = str_split($strTemp);
                return $chrTemp[0] . "<br />" . $chrTemp[1] . "<br />";
            }
        }
        ;
        return "";
    ...
    //複合仮名（二文字）半角から全角転換
    public function toTwoChar($str)
    {
        $strTempFull = "";
        $strTempHalf = "";

        $strTempFull = "キャキュキョシャシュショチャチュチョニャニュニョヒャヒュヒョミャミュミョリャリュリョ";
        $strTempHalf = "ｷｬｷｭｷｮｼｬｼｭｼｮﾁｬﾁｭﾁｮﾆｬﾆｭﾆｮﾋｬﾋｭﾋｮﾐｬﾐｭﾐｮﾘｬﾘｭﾘｮ";
        for ($i = 0; $i < mb_strlen($strTempHalf, "UTF-8"); $i += 2) {
            $str2 = "";
            $str2 = substr($strTempHalf, $i, 2);
            if ($str == $str2) {
                return substr($strTempFull, $i, 2) . "<br />";
            }
        }
        ;
        return "";
    ...
    public function toOneChar($str)
    {
        ...
        for ($i = 0; $i < mb_strlen($strTempHalf, "UTF-8"); $i += 2) {
            ...
            $str2 = substr($strTempHalf, $i, 2);
            if ($str == $str2) {
                return substr($strTempFull, $i / 2, 1) . "<br />";
            }
        }
        ;
        return "";
    ...
    public function toChar($str)
    {
        ...
        $strTempFull = "アイウエオサシスセソカキクケコタチツテトナニヌネノハヒフヒホマミムメモヤユヨラリルレロワヲン";
        $strTempHalf = "ｱｲｳｴｵｻｼｽｾｿｶｷｸｹｺﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾋﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜｦﾝ";
        for ($i = 0; $i < mb_strlen($strTempHalf, "UTF-8"); $i++) {
            $str2 = "";
            $str2 = substr($strTempHalf, $i, 1);
            if ($str == $str2) {
                return substr($strTempFull, $i, 1) . "<br />";
            }
        }
        ;
        return "";
    ...
    function HkToFk($str)
    {
        //配列を用意する
        //全角
        $zenkaku = array("ギャ", "ギ", "グ", "ゲ", "ゴ", "ザ", "ジ", "ズ", "ゼ", "ゾ", "ダ", "ヂ", "ヅ", "デ", "ド", "バ", "パ", "ビ", "ピ", "ブ", "プ", "ベ", "ペ", "ボ", "ポ", "ヴ", "ァ", "ア", "ィ", "イ", "ゥ", "ウ", "ェ", "エ", "ォ", "オ", "カ", "キ", "ク", "ケ", "コ", "サ", "シ", "ス", "セ", "ソ", "タ", "チ", "ッ", "ツ", "テ", "ト", "ナ", "ニ", "ヌ", "ネ", "ノ", "ハ", "ヒ", "フ", "ヘ", "ホ", "マ", "ミ", "ム", "メ", "モ", "ャ", "ヤ", "ュ", "ユ", "ョ", "ヨ", "ラ", "リ", "ル", "レ", "ロ", "ワ", "ヲ", "ン", "｜", "｜", "∧", "∨", "｜", "ー", "｜", "゜");
        //半角
        $hankaku = array("ｷﾞｬ", "ｷﾞ", "ｸﾞ", "ｹﾞ", "ｺﾞ", "ｻﾞ", "ｼﾞ", "ｽﾞ", "ｾﾞ", "ｿﾞ", "ﾀﾞ", "ﾁﾞ", "ﾂﾞ", "ﾃﾞ", "ﾄﾞ", "ﾊﾞ", "ﾊﾟ", "ﾋﾞ", "ﾋﾟ", "ﾌﾞ", "ﾌﾟ", "ﾍﾞ", "ﾍﾟ", "ﾎﾞ", "ﾎﾟ", "ｳﾞ", "ｧ", "ｱ", "ｨ", "ｲ", "ｩ", "ｳ", "ｪ", "ｴ", "ｫ", "ｵ", "ｶ", "ｷ", "ｸ", "ｹ", "ｺ", "ｻ", "ｼ", "ｽ", "ｾ", "ｿ", "ﾀ", "ﾁ", "ｯ", "ﾂ", "ﾃ", "ﾄ", "ﾅ", "ﾆ", "ﾇ", "ﾈ", "ﾉ", "ﾊ", "ﾋ", "ﾌ", "ﾍ", "ﾎ", "ﾏ", "ﾐ", "ﾑ", "ﾒ", "ﾓ", "ｬ", "ﾔ", "ｭ", "ﾕ", "ｮ", "ﾖ", "ﾗ", "ﾘ", "ﾙ", "ﾚ", "ﾛ", "ﾜ", "ｦ", "ﾝ", "-", "－", "(", ")", "ｰ", "ｰ", "ー", "ﾟ");
        //変換開始
