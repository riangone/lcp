<?php
// App::uses('ClsComFncJKSYS', 'Model/JKSYS/Component');
...
class ClsComFncJKSYSComponent extends Component
{
    //メンバー変数宣言
    public $xml = "";
    public $xml_path = "";
    ...
    //**********************************************************************
    public function FncGetBusyoMstValue($strCode, &$objBusyoMst)
    {
        $objDR = "";
        $result = array();
        $Do_Execute = array();
        try {
            ...
            $Do_Execute = $this->ClsComFncJKSYS->FncGetBusyoMstValue($strCode);
            if (!$Do_Execute['result']) {
                throw new \Exception($Do_Execute['data']);
            }
            //** ＳＱＬ実行＆構造体へ格納
            ...
            if (count((array) $objDR) > 0) {
                ...
                $objBusyoMst['strKKRBusyo'] = $this->FncNv($objDR[0]["BUSYO_CD"]);
            } else {
                //--------------------
                //　該当データ無
                //--------------------
    ...
    public function japDateChange($INDATE, $FORMAT)
    {
        ...
        try {
            ...
            $this->ClsComFnc = new ClsComFncJKSYS();
