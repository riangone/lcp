...
// * 処理説明	：共通クラスの読込み
//*************************************
class FrmPPRMMainMenu extends ClsComDb
{
    //部署のチェック
    ...
    function fncBusyoKBSql($strBusyoCD)
    {
        ...
        $strSQL = str_replace("@BUSYO_CD", $strBusyoCD != null ? $strBusyoCD : '', $strSQL);
        return $strSQL;
    }

    //---20170710 li INS S.
    //PPRMのSession 設定のSQL
    function menulistSql15($user_id, $sys_kb)
    {
        $strSQL = "";
    ...
    //PPRMのSession 設定
    public function getmenulist15($user_id, $sys_kb)
    {
        $res = parent::select($this->menulistSql15($user_id, $sys_kb));
        return $res;
    }
    ...
    {
        $strSql = "SELECT sys_cd FROM system_m WHERE sys_key='PPRM'";
        // echo $strSql;
        return $strSql;
    }
