...
// * 処理説明	：機能名のデータ処理クラス
//*************************************
class HDKReOut4OBC extends ClsComDb
{
    function FncGetSql_CSV($postData)
    {
        $strSQL = "";
        $strSQL .= "SELECT " . "\r\n";
        $strSQL .= "       HDPOUTGROUPDATA_OBC.CSV_GROUP_NO AS GROUP_NO" . "\r\n";
        $strSQL .= ",      SUBSTRB(HDPOUTGROUPDATA_OBC.CSV_GROUP_NM,1,24) AS CSV_GROUP_NM" . "\r\n";
        $strSQL .= ",      TO_CHAR(HDPOUTGROUPDATA_OBC.CSV_OUT_DT,'YYYY/MM/DD HH24:MI:SS') AS CSV_OUT_DT" . "\r\n";
        $strSQL .= ",      TO_CHAR(HDPOUTGROUPDATA_OBC.UPD_DATE,'YYYYMMDD') AS GROUPDATE" . "\r\n";
        $strSQL .= ",      SUBSTR(HDPOUTGROUPDATA_OBC.KEIRI_DT,1,4) || '/' || SUBSTR(HDPOUTGROUPDATA_OBC.KEIRI_DT,5,2) || '/' || SUBSTR(HDPOUTGROUPDATA_OBC.KEIRI_DT,7,2) AS KEIRI_DT" . "\r\n";
        $strSQL .= ",      SUM(HDPSHIWAKEDATA.ZEIKM_GK) AS SUMMONEY" . "\r\n";
        $strSQL .= " FROM " . "\r\n";
        $strSQL .= "      HDPOUTGROUPDATA_OBC " . "\r\n";
        $strSQL .= ",     HHAIZOKU " . "\r\n";
        $strSQL .= ",     HDPSHIWAKEDATA " . "\r\n";
        $strSQL .= " WHERE " . "\r\n";
        $strSQL .= "     HHAIZOKU.SYAIN_NO = HDPOUTGROUPDATA_OBC.UPD_SYA_CD" . "\r\n";
        $strSQL .= " AND HHAIZOKU.START_DATE <= TO_CHAR(HDPOUTGROUPDATA_OBC.UPD_DATE,'yyyymmdd')" . "\r\n";
        $strSQL .= " AND NVL(HHAIZOKU.END_DATE,'99999999') >= TO_CHAR(HDPOUTGROUPDATA_OBC.UPD_DATE,'yyyymmdd')" . "\r\n";
        $strSQL .= " AND HDPOUTGROUPDATA_OBC.CSV_GROUP_NO = HDPSHIWAKEDATA.XLSX_GROUP_NO" . "\r\n";
        ...
        if ($postData['ltxtTantouCD'] != '') {
            $strSQL .= "AND   HDPOUTGROUPDATA_OBC.UPD_SYA_CD ='@TANTOUCD' " . "\r\n";
        ...
        $strSQL .= " GROUP BY  " . "\r\n";
        $strSQL .= "       HDPOUTGROUPDATA_OBC.CSV_GROUP_NO,HDPOUTGROUPDATA_OBC.CSV_GROUP_NM, HDPOUTGROUPDATA_OBC.CSV_OUT_DT,HDPOUTGROUPDATA_OBC.UPD_DATE " . "\r\n";
        $strSQL .= " ,    HDPOUTGROUPDATA_OBC.KEIRI_DT " . "\r\n";
        $strSQL .= "  ORDER BY  " . "\r\n";
        $strSQL .= "   HDPOUTGROUPDATA_OBC.CSV_GROUP_NO DESC  " . "\r\n";
    ...
    function fncSelGroupShiwakeDataSql($postData)
    {
        ...
        //借方科目
        $strSQL .= ",      KAR.KAMOK_NAME L_KAMOKU,KAR.SUB_KAMOK_NAME L_KOUMKU" . "\r\n";
        //貸方科目
        ...
        $strSQL .= "         WHEN '1' THEN KAS.KAMOK_NAME" . "\r\n";
        $strSQL .= "         ELSE HMS.MEISYOU END  R_KAMOKU" . "\r\n";
        $strSQL .= ",      CASE BASE.DENPY_KB  " . "\r\n";
        $strSQL .= "         WHEN '1' THEN KAS.SUB_KAMOK_NAME" . "\r\n";
        $strSQL .= "         ELSE HMS.MOJI1 END R_KOUMKU" . "\r\n";
        ...
        $strSQL .= ",      ROW_NUMBER() OVER(ORDER BY BASE.XLSX_OUT_ORDER, BASE.SYOHY_NO) - 1 RENBAN" . "\r\n";
        $strSQL .= ",      ROW_NUMBER() OVER(ORDER BY BASE.XLSX_OUT_ORDER, BASE.SYOHY_NO) SEQNO" . "\r\n";
        ...
        $strSQL .= "		  WHERE  XLSX_GROUP_NO = @GROUPNO" . "\r\n";
        ...
        $strSQL .= " ORDER BY BASE.XLSX_OUT_ORDER " . "\r\n";
    ...
    function fncSelSyohyShiwakeDataSql($postData)
    {
        ...
        $strSQL .= "ORDER BY BASE.XLSX_OUT_ORDER,BASE.SYOHY_NO" . "\r\n";
    ...
    {
        $strSQL = "";
        $strSQL .= "SELECT CSV_GROUP_NM FROM HDPOUTGROUPDATA_OBC WHERE CSV_GROUP_NO = '" . $postData['strGroup_no'] . "'" . "\r\n";
    ...
    public function fncCheckOffGroup($strSyohyoNo)
    {
        ...
        $strSQL .= ",        A.XLSX_GROUP_NO AS グループ番号" . "\r\n";
    ...
    public function FncChkAndSetShiwakeInfoSql($strSyohyoNo)
    {
        ...
        $strSQL .= "             ELSE '0'" . "\r\n";
        $strSQL .= "         END AS 特別XLSXフラグ," . "\r\n";
        $strSQL .= "         B.XLSX_OUT_FLG AS XLSX出力フラグ," . "\r\n";
        $strSQL .= "         A.DEL_FLG AS 削除フラグ," . "\r\n";
    ...
    {
        $strSQL = "";
        $strSQL .= "UPDATE HDPOUTGROUPDATA_OBC SET" . "\r\n";
        ...
        $strSQL = str_replace("@UPD_PRG_ID", 'XlsxReOut', $strSQL);
    ...
    public function SubUpdateSyohyoData($postData, $strSyohyoNo, $strEdaNo, $groupNo, $sysDate, $intCsvOutOrd, $patternID, $BusyoCD)
    {
        ...
        $strSQL .= "XLSX_OUT_ORDER ='@intCsvOutOrd'," . "\r\n";
