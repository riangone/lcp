...
// * 処理説明	：機能名のデータ処理クラス
//*************************************
class HMDPS703SyainSearch extends ClsComDb
{
    ...
    function FncGetSql_SYAYIN($postData)
    {
        ...
        if ($postData['txtSyainNO'] != '') {
            ...
            $strSQL = str_replace("@SYAIN_NO", $this->ClsComFncHMDPS->FncNv($postData['txtSyainNO']), $strSQL);
        }
        if ($postData['txtSyainNM'] != '') {
            $strSQL .= "   AND   SYA.SYAIN_NM      LIKE '@SYAIN_NM%'";
            $strSQL = str_replace("@SYAIN_NM", $this->ClsComFncHMDPS->FncNv($postData['txtSyainNM']), $strSQL);
        }
        if ($postData['txtSyainKN'] != '') {
            $strSQL .= "   AND   SYA.SYAIN_KN      LIKE '@SYAIN_KN%'";
            $strSQL = str_replace("@SYAIN_KN", $this->ClsComFncHMDPS->FncNv($postData['txtSyainKN']), $strSQL);
        }
        if ($postData['txtBusyoCD'] != '') {
            $strSQL .= "   AND   HAI.BUSYO_CD      = '@BUSYO_CD'";
            $strSQL = str_replace("@BUSYO_CD", $this->ClsComFncHMDPS->FncNv($postData['txtBusyoCD']), $strSQL);
        }
        $strSQL .= "  ORDER BY ";
        $strSQL .= "       SYA.SYAIN_NO ";
