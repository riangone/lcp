...
class FrmSinKaverRankHyoKRSS extends ClsComDb
{
    protected $conn_orl = "";
    protected $Sel_Array = "";
    public $ClsComFnc;
    ...
    function fncUriageRankSelSql($postData = NULL)
    {
        ...
        $strSQL .= "		,      SUM(NVL(SLS.SIN_DAISU,0)) TOUKI_DAISU" . "\r\n";
        $strSQL .= "		,      SUM(CASE WHEN SLS.KEIJO_DT = '@TUKI' THEN NVL(SLS.SIN_DAISU,0) ELSE 0 END) TOUGETU_DAISU" . "\r\n";
    ...
    function fncKaverRankSelSql($postData, $intMemoCnt)
    {
        ...
        $strSQL .= ",      TRUNC((ROW_NUMBER() OVER(ORDER BY RT_TBL.KOTEI_KAVER_RT DESC) - 0.1) / (97 - @MEMOCNT)) ROW_CNT" . "\r\n";
        ...
        $strSQL .= "             ,      MAX(CASE WHEN SLS.KEIJO_DT = '@TUKI' THEN NVL(SLS.KNR_DAISU,0) ELSE 0 END) KANRI_DAISU" . "\r\n";

        $strSQL .= "             ,      MAX(CASE WHEN SLS.KEIJO_DT = '@TUKI' THEN NVL(SLS.KEI_NEN_SU,0) ELSE 0 END) KEIKEN_NENSU" . "\r\n";
        ...
        if ($postData['radBusyoCheck'] == 'true' || $postData['radRankingCheck'] == 'true') {
            $strSQL .= "ORDER BY RT_TBL.KOTEI_KAVER_RT DESC, RT_TBL.BUSYO_CD ASC, RT_TBL.SYAIN_NO ASC " . "\r\n";
        } else {
            $strSQL .= "ORDER BY RT_TBL.Y_MINUS_KAVER_RT DESC, RT_TBL.BUSYO_CD ASC, RT_TBL.SYAIN_NO ASC " . "\r\n";
        }
        ...
        $strSQL = str_replace("@TUKI", substr(str_replace("/", "", $postData['cboYMEnd']), 0, 6), $strSQL);
        $strSQL = str_replace("@JUNI", $postData['Rank'], $strSQL);
        $strSQL = str_replace("@MEMOCNT", $intMemoCnt, $strSQL);
        return $strSQL;
    }
    ...
    function fncStandardInfoSelSql($postData = NULL)
    {
        ...
        $strSQL = "";
        $strSQL .= "SELECT " . "\r\n";
        $strSQL .= "       '第@JIKI期営業スタッフ総限界利益・固定費カバー率ランキング表' || '@SUBTITLE' TITLE" . "\r\n";
    ...
    public function fncKaverRankSel($postData, $intMemoCnt)
    {
        $strSql = $this->fncKaverRankSelSql($postData, $intMemoCnt);
