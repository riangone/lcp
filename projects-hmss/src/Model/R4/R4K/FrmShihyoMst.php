...
class FrmShihyoMst extends ClsComDb
{
    ...
    // '**********************************************************************
    // '処 理 名：社員ﾏｽﾀから指標ファイルの基本情報を抽出する
    // '関 数 名：fncShihyoSelectSQL
    // '引    数：無し
    // '戻 り 値：ＳＱＬ文
    // '処理説明：社員ﾏｽﾀから指標ファイルの基本情報を抽出する
    // '**********************************************************************
    function fncShihyoSelectSQL($BUSYOCD, $KI)
    {
        $strSQL = "";
    ...
    function frmHSHIHYODeleteRowSQL($BUSYOCD, $KI, $LINENO)
    {
        $strSQL = "";

        $strSQL = "DELETE FROM HSHIHYO WHERE KI = ";
    ...
    {
        $strSQL = "";
        $strSQL = "DELETE FROM HSHIHYO";
        $strSQL .= "   WHERE  BUSYO_CD = '@BUSYO'";
        $strSQL .= "   AND    KI = @KI";

        $strSQL = str_replace("@BUSYO", rtrim($BUSYOCD), $strSQL);
        $strSQL = str_replace("@KI", (substr($KI, 0, 4)) - 1917, $strSQL);
    ...
    function fncInsertYosanMst($BUSYOCD, $KI, $KKRBUSYO, $INPUTDATA, $LINENO)
    {
        ...
        $strSQL .= ",      '@BUSYOCD'";
        $strSQL .= ",     " . ($LINENO + 1);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['UPD_FPG']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK10']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK11']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK12']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK1']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK2']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK3']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK4']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK5']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK6']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK7']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK8']), "", 2);
        $strSQL .= "," . $this->FncSqlNv2(rtrim($INPUTDATA['YSN_GK9']), "", 2);
        $strSQL .= ", SYSDATE";
        ...
        $strSQL = str_replace("@KKRBUSYO", rtrim($KKRBUSYO), $strSQL);
    ...
    public function fncShihyoSelect($BUSYOCD, $KI)
    {
        $strsql = $this->fncShihyoSelectSQL($BUSYOCD, $KI);
        return parent::select($strsql);
    }

    public function frmHSHIHYODeleteRow($BUSYOCD, $KI, $LINENO)
    {
        $strsql = $this->frmHSHIHYODeleteRowSQL($BUSYOCD, $KI, $LINENO);
