 ...
 *	http://jquery.com
 *  Download url：https://cyokodog.hatenadiary.org/entry/20090316/jQueryExDate
 *
 */
(function ($j) {
    $j.ex = $j.ex || {};
    ...
    $j.exDate = function (dateStr, format) {
        if (!(this instanceof $j.exDate)) return new $j.exDate(dateStr, format);
    ...
    $j.extend($j.exDate.prototype, {
        toDate: function (dateStr, format) {
            ...
            if (dateStr != undefined) {
                ...
                (format || o._defaultFormat).replace(
                    o._exprMatch,
                    function (key, pos, format) {
                        var num =
                            dateStr.substr(
                                pos,
                                key.replace("hh24", "hh").length
                            ) - 0;
                        var exp = o._expr[key];
                        if (exp)
                            tmp[exp.type] = exp.toDate ? exp.toDate(num) : num;
                    }
                );
        ...
        toChar: function (format) {
            var o = this;
            return (format || o._defaultFormat).replace(
                o._exprMatch,
                function (key, pos, format) {
        ...
        lastDay: function () {
            ...
            var d = $j.exDate(
                $j.ex.lpad(y, 4, "0") + "/" + $j.ex.lpad(m, 2, "0") + "/" + "01"
            );
