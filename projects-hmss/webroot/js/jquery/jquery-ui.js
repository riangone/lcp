 ...
 * https://code.jquery.com/1.13.3/jquery-ui.js */
...
(function (factory) {
    ...
    $.widget("ui.button", {
        ...
        _create: function () {
            ...
            this._setOption("disabled", this.options.disabled);
            this._enhance();
            //20210128 YIN INS S [SDH button icon highlight when hover]
    ...
    function Ympicker() {
        ...
        this._datepickerShowing = false; // True if the popup picker is showing , false if not
        this._inDialog = false; // True if showing within a "dialog", false if not
        this._mainDivId = "ui-ympicker-div"; // The ID of the main datepicker division
        this._inlineClass = "ui-ympicker-inline"; // The name of the inline marker class
        this._appendClass = "ui-ympicker-append"; // The name of the append marker class
        this._triggerClass = "ui-ympicker-trigger"; // The name of the trigger marker class
        this._dialogClass = "ui-ympicker-dialog"; // The name of the dialog marker class
        this._disableClass = "ui-ympicker-disabled"; // The name of the disabled covering marker class
        this._unselectableClass = "ui-ympicker-unselectable"; // The name of the unselectable cell marker class
        this._currentClass = "ui-ympicker-current-day"; // The name of the current day marker class
        this._dayOverClass = "ui-ympicker-days-cell-over"; // The name of the day hover marker class
        this.regional = []; // Available regional settings, indexed by language code
        this.regional[""] = {
            // Default regional settings
            closeText: "Done", // Display text for close link
    ...
    $.extend(Ympicker.prototype, {
        ...
        _attachments: function (input, inst) {
            ...
            if (showOn === "button" || showOn === "both") {
                ...
                inst.trigger.on("click", function () {
                    if (
                        $.ympicker._datepickerShowing &&
                        $.ympicker._lastInput === input[0]
                    ) {
                        ...
                        $.ympicker._hideDatepicker();
                        $.ympicker._showDatepicker(input[0]);
                    } else {
                        $.ympicker._showDatepicker(input[0]);
                    }
                    return false;
            ...
            if ($.ympicker._datepickerShowing) {
                switch (event.keyCode) {
                    ...
                    case 13:
                        sel = $(
                            "td." +
                                $.ympicker._dayOverClass +
                                ":not(." +
                                $.ympicker._currentClass +
                                ")",
                            inst.dpDiv
                        );
                        if (sel[0]) {
                    ...
                    case 27:
                        ...
                        break; // hide on escape
                    case 33:
                        $.ympicker._adjustDate(
                            event.target,
                            event.ctrlKey
                                ? -$.ympicker._get(inst, "stepBigMonths")
                                : -$.ympicker._get(inst, "stepMonths"),
                            "M"
                        );
                        break; // previous month/year on page up/+ ctrl
                    case 34:
        ...
        _doKeyPress: function (event) {
            ...
            if ($.ympicker._get(inst, "constrainInput")) {
                chars = $.ympicker._possibleChars(
                    $.ympicker._get(inst, "dateFormat")
                );
                chr = String.fromCharCode(
                    event.charCode == null ? event.keyCode : event.charCode
                );
        ...
        _showDatepicker: function (input) {
            ...
            if (
                $.ympicker._isDisabledDatepicker(input) ||
            ...
            if (!inst.inline) {
                ...
                if ($.effects && $.effects.effect[showAnim]) {
                    inst.dpDiv.show(
                        showAnim,
                        $.ympicker._get(inst, "showOptions"),
                        duration
                    );
                } else {
                    inst.dpDiv[showAnim || "show"](showAnim ? duration : null);
        ...
        _updateDatepicker: function (inst) {
            ...
            var origyearshtml,
                ...
                width = 17,
                activeCell = inst.dpDiv.find("." + this._dayOverClass + " a"),
                onUpdateDatepicker = $.ympicker._get(
                    inst,
                    "onUpdateDatepicker"
                );
            ...
            if (
                inst === $.ympicker._curInst &&
                $.ympicker._datepickerShowing &&
                $.ympicker._shouldFocusInput(inst)
            ) {
                inst.input.trigger("focus");
            }
        ...
        _checkExternalClick: function (event) {
            ...
            if (
                ($target[0].id !== $.ympicker._mainDivId &&
                    ...
                    while (matches) {
                        switch (matches[2] || "d") {
                            ...
                            case "M":
                                ...
                                day = Math.min(
                                    day,
                                    $.ympicker._getDaysInMonth(year, month)
                                );
                                break;
                            case "y":
                            case "Y":
                            ...
                            tbody +=
                                ...
                                (otherMonth && !showOtherMonths
                                    ...
                                    : "<a class='ui-state-default" +
                                      ...
                                      printDate.getDate() + // store date as data
                                      "'>" +
                                      monthNamesShort[printDate.getMonth()] +
                                      "</a>") +
                                "</td>"; // display selectable date
    ...
    function ympicker_handleMouseover() {
        if (
            !$.ympicker._isDisabledDatepicker(
                datepicker_instActive.inline
                    ? datepicker_instActive.dpDiv.parent()[0]
                    : datepicker_instActive.input[0]
            )
    ...
    $.fn.ympicker = function (options) {
        ...
        if (
            ...
            return $.ympicker["_" + options + "Datepicker"].apply(
                $.ympicker,
                [this[0]].concat(otherArgs)
            );
        }
        ...
        return this.each(function () {
            if (typeof options === "string") {
                $.ympicker["_" + options + "Datepicker"].apply(
                    $.ympicker,
                    [this].concat(otherArgs)
                );
            } else {
