...
EscapeSJIS = function (str) {
    return str.replace(/[^*+.-9A-Z_a-z-]/g, function (s) {
        ...
        return c < 128
            ...
            : "%" +
              ((m =
                  ((c < 8272 ? c : (c = JCT11280.lastIndexOf(s))) -
                      (c %= 188)) /
                  188) < 31
                  ? m + 129
                  : m + 193
...
UnescapeEUCJP = function (str) {
    return str.replace(
        /(%A[1-9A-F]|%[B-E][0-9A-F]|%F[0-9A-E]){2}|%8E%(A[1-9A-F]|[B-D][0-9A-F])|%[0-7][0-9A-F]/gi,
        function (s) {
            var c = parseInt(s.substring(1), 16);
            return c < 161
                ? String.fromCharCode(
                      c < 128 ? c : parseInt(s.substring(4), 16) + 65216
                  )
                : JCT8836.charAt(
                      (c - 161) * 94 + parseInt(s.substring(4), 16) - 161
                  );
        }
    );
};
...
EscapeJIS7 = function (str) {
    ...
    return ("g" + str + "g")
        .replace(RegExp("[" + ri + "]+", "g"), function (s) {
...
UnescapeJIS7 = function (str) {
    var i = 0,
        ...
        P = (
            ...
            str
                .replace(/%49/g, "I")
                .replace(/%1B%24%4[02]|%1B%24@/gi, "%1B%24B")
        ).split(/%1B/i),
        I = function (s) {
            ...
            return JCT8836.charAt(
                4 < l
                    ? (parseInt(s.substring(1), 16) - 33) * 94 +
                          parseInt(s.substring(4), 16) -
                          33
                    : 2 < l
                    ? (37 == (l = s.charCodeAt(0))
                          ? (parseInt(s.substring(1, 3), 16) - 33) * 94 +
                            s.charCodeAt(3)
...
UnescapeJIS8 = function (str) {
    var i = 0,
        p,
        s = "",
        P = ("%28B" + str.replace(/%1B%24%4[02]|%1B%24@/gi, "%1B%24B")).split(
            /%1B/i
        ),
        I = function (s) {
            var c = parseInt(s.substring(1), 16);
...
UnescapeUnicode = function (str) {
    return str.replace(/%u[0-9A-F]{4}|%[0-9A-F]{2}/gi, function (s) {
...
EscapeUTF7 = function (str) {
    ...
    re.compile("[^+]{1,3}", "g");
    return (str + "g")
        .replace(/[^*+.-9A-Z_a-z-]+[*+.-9A-Z_a-z-]|[+]/g, function (s) {
            if ("+" == s) return "+-";
            var l = s.length - 1,
                w = s.charAt(l);
            return (
                "+" +
                s.substring(0, l).replace(re, E) +
                ("+" == w
                    ? "-+-"
                    : "*" == w || "." == w || "_" == w
                    ? w
        ...
        return (
            c < 16
                ...
                ? "%" +
                  ((c >> 6) | 192).toString(16) +
                  "%" +
                  ((c & 63) | 128).toString(16)
                : "%" +
                  ((c >> 12) | 224).toString(16) +
                  "%" +
                  (((c >> 6) & 63) | 128).toString(16) +
                  "%" +
                  ((c & 63) | 128).toString(16)
        ).toUpperCase();
    });
...
UnescapeUTF8 = function (str) {
    return str.replace(
        /%(E(0%[AB]|[1-CEF]%[89AB]|D%[89])[0-9A-F]|C[2-9A-F]|D[0-9A-F])%[89AB][0-9A-F]|%[0-7][0-9A-F]/gi,
        function (s) {
            var c = parseInt(s.substring(1), 16);
...
GetEscapeCodeType = function (str) {
    ...
    if (
        /%([0-9A-DF][0-9A-F]%[8A]0%|E0%80|[0-7][0-9A-F]|C[01])%[8A]0|%00|%[7F]F/i.test(
            str
        )
    )
        return "UTF16LE";
    ...
    if (/%F[DE]/i.test(str))
        return /%8[0-9A-D]|%9[0-9A-F]|%A0/i.test(str) ? "UTF16LE" : "EUCJP";
    if (/%1B/i.test(str)) return /%[A-D][0-9A-F]/i.test(str) ? "JIS8" : "JIS7";
    var S = str
            .substring(0, 6143)
            .replace(/%[0-9A-F]{2}|[^ ]| /gi, function (s) {
