 ...
 * ----------------------------------------------------------------------------------------------------------------------------------
 */
Namespace.register("R4.FrmNinhoIn");

R4.FrmNinhoIn = function () {
    // ==========
    // = 宣言 start =
    // ==========
    ...
    me.controls.push({
        id: ".FrmNinhoIn.cmdOpen",
        type: "button",
        handle: "",
    });

    me.controls.push({
        id: ".FrmNinhoIn.cmdAct",
        type: "button",
        handle: "",
    });
    ...
    $(".FrmNinhoIn.cmdAct").click(function () {
        if (me.fileMark == 0) {
            me.fncCheckFile();
        } else {
    ...
    $(".FrmNinhoIn.cmdOpen").click(function () {
        //参照ボタンcmdOpen_Click
        $(".FrmNinhoIn.txtFile").val("");
        me.fu = new gdmz.common.file();
        ...
        $("#file").change(function () {
            ...
            if (this.files[i].size > 2048000) {
                ...
                $(".FrmNinhoIn.txtFile").trigger("focus");
                return;
            }
            ...
            $(".FrmNinhoIn.txtFile").val(this.files[i].name);
    ...
    $(".FrmNinhoIn.cboYM").on("blur", function () {
        //-- 20150922 Yuanjh UPD S.
        //if (clsComFnc.CheckDate2($(".FrmNinhoIn.cboYM")) == false)
        //-- 20150922 Yuanjh UPD E.
        if (clsComFnc.CheckDate3($(".FrmNinhoIn.cboYM")) == false) {
            //20201117 wangying ins S
            window.setTimeout(function () {
                //20201117 wangying ins E
                $(".FrmNinhoIn.cboYM").val(me.cboYM);
                $(".FrmNinhoIn.cboYM").trigger("focus");
                $(".FrmNinhoIn.cboYM").select();
                $(".FrmNinhoIn.cmdAct").button("disable");
            ...
            $(".FrmNinhoIn.cmdAct").button("enable");
        }
    });

    // $(".FrmNinhoIn.cboYM").bind('keydown', function(e)
    // {
    // var key = e.which;
    ...
    // {
    // $(".FrmNinhoIn.cmdAct").button('enable');
    // $(".FrmNinhoIn.cmdAct").trigger("focus");
    // }
    // });
    // ==========
    // = イベント end =
    ...
    me.cmdAct_Click = function () {
        ...
        //-- 20150922 Yuanjh UPD S.
        //var KEIJOBIVal = $(".FrmNinhoIn.cboYM").val();
        var KEIJOBIVal =
            $(".FrmNinhoIn.cboYM").val().substr(0, 4) +
            "/" +
            $(".FrmNinhoIn.cboYM").val().substr(4, 2);
        //-- 20150922 Yuanjh UPD E.

        var FileName = $(".FrmNinhoIn.txtFile").val();
    ...
    me.fncCheckFile = function () {
        ...
        if (FileName.trimEnd() == "") {
            clsComFnc.ObjFocus = $(".FrmNinhoIn.txtFile");
            MessageBox.MessageBox(
                "取込ﾌｧｲﾙを指定してください。",
                "HMReports",
                "OK",
    ...
    me.frmSample_Load = function () {
        ...
        ajax.receive = function (result) {
            ...
            //-- 20150922 Yuanjh UPD S.
            //$(".FrmNinhoIn.cboYM").val(tmpNowDate);
            $(".FrmNinhoIn.cboYM").val(
                myDate.getFullYear().toString() + tmpMonth.toString()
            );
            //-- 20150922 Yuanjh UPD E.
            ...
            if (result["row"] == 0) {
                ...
                //-- 20150922 Yuanjh UPD S.
                //$(".FrmNinhoIn.cboYM").val(strTougetu[0] + '/' + strTougetu[1]);
                $(".FrmNinhoIn.cboYM").val(strTougetu[0] + strTougetu[1]);
...
$(function () {
    var o_R4_FrmNinhoIn = new R4.FrmNinhoIn();
    o_R4_FrmNinhoIn.load();
});
