 ...
 * @author FCSDL
 */
Namespace.register("HMAUD.HMAUD");

HMAUD.HMAUD = function () {
    ...
    me.CONST_MANAGER_PTN_NO = "002";
    me.CONST_TESTER_PTN_NO = "999";
    ...
    me.load = function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                $(".HMAUD.HMAUD-layout-west").html(result);
                $(".HMAUD.HMAUD-loading-icon").hide();
                $(".ui-widget-content.HMAUD.HMAUD-layout-west").css(
                    "overflow",
                    "scroll"
                );
            },
    ...
    me.EnterKeyDown = function (form_id) {
        ...
        $inp.on("keydown", function (e) {
            var key = e.which;
            if (key == 13) {
                if (this.type != "submit" && this.type != "textarea") {
                    e.preventDefault();
    ...
    me.halfToFull = function (str) {
        ...
        for (var i = 0; i < str.length; i++) {
            ...
            strTemp = me.toThreeChar(str.substring(i, i + 3));
            if (strTemp != "") {
                fullStr = fullStr + strTemp;
                i += 2;
            } else {
                //複合仮名（二文字キャ）半角から全角転換
                strTemp = me.toTwoChar(str.substring(i, i + 2));
                if (strTemp != "") {
                    fullStr = fullStr + strTemp;
                    i += 1;
                } else {
                    //仮名（二文字ガ）半角から全角転換
                    strTemp = me.toOneChar(str.substring(i, i + 2));
                    if (strTemp != "") {
                        fullStr = fullStr + strTemp;
                        i += 1;
                    } else {
                        //普通仮名（1文字ア）半角から全角転換
                        strTemp = me.toChar(str.substring(i, i + 1));
                        if (strTemp != "") {
                            fullStr = fullStr + strTemp;
                        } else {
                            //横棒(－)から縦棒(｜)転換
                            strTemp = me.toLine(str.substring(i, i + 1));
                            if (strTemp != "") {
                                fullStr = fullStr + strTemp;
                            } else {
                                //仮名以外1文字
                                strTemp = $.trim(str.substring(i, i + 1));
                                if (strTemp != "") {
                                    fullStr =
                                        fullStr +
                                        str.substring(i, i + 1) +
    ...
    me.toThreeChar = function (str) {
        ...
        for (var i = 0; i < strTempHalf.length; i += 3) {
            var str2 = "";
            count += 1;
            str2 = strTempHalf.substring(i, i + 3);
            if (str === str2) {
                // var strTemp = strTempFull.substr(count * 2 - 2, 2);
    ...
    me.toTwoChar = function (str) {
        ...
        for (var i = 0; i < strTempHalf.length; i += 2) {
            var str2 = "";
            str2 = strTempHalf.substring(i, i + 2);
            if (str === str2) {
                return strTempFull.substring(i, i + 2) + "<br/>";
            }
        }
        return "";
    };
    ...
    me.toOneChar = function (str) {
        ...
        for (var i = 0; i < strTempHalf.length; i += 2) {
            ...
            str2 = strTempHalf.substring(i, i + 2);
            if (str === str2) {
                return strTempFull.substring(i / 2, i / 2 + 1) + "<br/>";
            }
        }
        return "";
    };
    ...
    me.toChar = function (str) {
        ...
        for (var i = 0; i < strTempHalf.length; i++) {
            var str2 = "";
            str2 = strTempHalf.substring(i, i + 1);
            if (str === str2) {
                return strTempFull.substring(i, i + 1) + "<br/>";
            }
        }
        return "";
    };
