(function ($) {
    ...
    var openedMenu = [];
    //fan add end.
    $.fn.mcDropdown = function (list, options, divid) {
        ...
        this.each(function () {
            ...
            new $.mcDropDownMenu(this, list, options, divid);
        });
    ...
    $.mcDropDownMenu = function (el, list, options, divid) {
        var $self,
            thismenu = this,
            $list,
            $divInput,
        ...
        if (settings.mouseintent !== false && !$.fn.mouseintent) {
            ...
            settings.mouseintent = $.extend(
                {},
                mouseintentDefaults,
                $.isPlainObject(settings.mouseintent)
        ...
        if (settings.click == null) {
            settings.click = function (e, dropdown, settings) {
                if (this.attr(settings.valueAttr)) {
                    ...
                    dropdown.setValue(
                        $(this.parents("li")[0]).attr(settings.valueAttr)
                    );
                }
            };
        }
        ...
        $(document)
            ...
            .on("click", function (e) {
                ...
                if ($ul.length) {
                    ...
                    else if (!settings.allowParentSelect && bIsParent)
                        return false;

                    // make sure to hide the parent branch if we're not the root
                    if ($target.not(".mcdropdown-item-root"))
                        hideBranch.apply($target.parent().parent()[0], [e]);

                    if (
                        settings.click != null &&
                        settings.click.apply($target, [
                            e,
                            thismenu,
                            settings,
                        ]) == false
        ...
        $divInput = $(
            '<div class="mcdropdown ' +
                ...
                '" id = "' +
                divid +
                '"><a href="#" tabindex="-1" class="mcdropdown-dropdown-handle"></a><input type="hidden" name="' +
                (el.name || el.id) +
                '" id="' +
                (el.id || el.name) +
                '" /></div>'
        ...
        var $input = $self
            .replaceWith($divInput)
            .attr({ id: "TXT" + divid, name: "TXT" + divid })
            .addClass("mcdropdown-input-control");
        ...
        $.data($hidden[0], "mcDropdown" + divid, thismenu);
        ...
        $("a", $divInput).on("click", function (e) {
        ...
        this.openMenu = function (e) {
            ...
            if (openedMenu.length != 0) {
                openedMenu.pop().closeMenu();
            }
            openedMenu.push(thismenu);
            //fan add end.
            ...
            function open() {
                ...
                $list
                    .find(".mcdropdown-item-hover")
                    .removeClass("mcdropdown-item-hover");
        ...
        function getNodeText($el) {
            ...
            var nContents = $el.contents().filter(function () {
                ...
                return (
                    this.nodeType == 1 ||
                    (this.nodeType == 3 && $.trim(this.nodeValue).length > 0)
                );
            });
        ...
        function addBindings(el) {
            removeBindings(el);
            $("> li", el)
                .on("mouseover.mcdropdown", hoverOver)
                .on("mouseout.mcdropdown", hoverOut);
        }

        function removeBindings(el) {
            // find the menu item
            $("> li", el).unbind(".mcdropdown." + divid);
        }
                ...
                $.data(
                    ...
                    setTimeout(function () {
                        ...
                        if (settings.mouseintent) {
                            ...
                            $child.on("mouseenter.mcdropdown", function () {
                                $(this).mouseintent(
                                    $.extend({}, settings.mouseintent, {
                                        mouseaway: function () {
                                            ...
                                            if (
                                                ...
                                                settings.mouseintent.mouseaway.apply(
                                                    this,
                                                    arguments
                                                );
        ...
        function hoverOut(e) {
            ...
            if (
                $child.length &&
                (settings.mouseintent === false || !$child.data("mouseintent"))
            ) {
                $.data(
                    self,
                    "mcDropdown-timer",
        ...
        function getShadow(depth) {
            ...
            if (!shadows[depth]) {
                // create shadow
                shadows[depth] = $(
                    '<div class="mcdropdown-shadow"></div>'
                ).appendTo("body");
        ...
        function showBranch() {
            ...
            var $li = $(this),
                li_coords = position($li, $li.css("position"));
        ...
        function hideBranch(force) {
            var $ul = getChildMenu(this);
            // if the menu is already visible or there is no submenu, cancel
            if (($ul.is(":hidden") || $ul.length == 0) && force !== true)
                return false;
        ...
        function position($el, type) {
            ...
            if (type === "relative") {
                ...
                pos = $.extend($el[type === true ? "offset" : "position"](), {
                    marginLeft: parseInt($($el[0]).css("marginLeft"), 10) || 0,
                    marginRight:
                        parseInt($($el[0]).css("marginRight"), 10) || 0,
                    marginTop: parseInt($($el[0]).css("marginTop"), 10) || 0,
                    marginBottom:
                        parseInt($($el[0]).css("marginBottom"), 10) || 0,
                });
            }
        ...
        function getListDimensions($el, cols) {
            if (!$el.data("dimensions")) {
                ...
                var width =
                    $el === $list && $el.data("width") * cols < ddWidth
        ...
        function columnizeList($el) {
            ...
            $children.each(function (i) {
                ...
                $li.css({
                    ...
                    [
                        nextItemColumn > column || currentItem == items
                            ? "addClass"
                            : "removeClass"
                    ]("mcdropdown-item-endcol")
                    [marginTop != 0 ? "addClass" : "removeClass"](
                        "mcdropdown-item-firstrow"
                    );
            ...
            if (
                $el !== $list &&
                maxColumnHeight + settings.screenPadding * 2 >=
        ...
        function onFocus(e) {
            ...
            if (!$keylist) {
                $keylist = $(
                    '<ul class="mcdropdown-autocomplete"></ul>'
                ).appendTo("body");
            }

            // should we show matches?
            var hideResults = !(
                settings.showACOnEmptyFocus && typedText.length == 0
            );

            // get the siblings for the current item
            var $siblings =
                $current.length == 0 ||
                $current.hasClass("mcdropdown-item-root")
                    ? $list.find("> li")
        ...
        function onBlur(e) {
            ...
            iBlurTimeout = setTimeout(function () {
                ...
                if (
                    !settings.allowParentSelect &&
                    $current.is(".mcdropdown-item-parent")
                ) {
                    // grab the first end child item we can find for the current path
                    var value = $current
                        .find("li:not('.mcdropdown-item-parent'):first")
        ...
        function showMatches($li, hideResults) {
            var bCached = $li === oldCache,
                $items = bCached
                    ? $keylist
                          .find("> li")
                          .removeClass(
                              "mcdropdown-item-hover mcdropdown-item-hover-parent mcdropdown-item-firstrow"
                          )
                    : $li
                          .clone()
                          .removeAttr("style")
                          .removeClass(
                              "mcdropdown-item-hover mcdropdown-item-hover-parent mcdropdown-item-firstrow mcdropdown-item-endcol"
                          )
                          .filter(":last")
                          .addClass("mcdropdown-item-endcol")
                          .end();
                ...
                $items.hover(
                    function () {
                        $keylist
                            .find("> li")
                            .removeClass(
                                "mcdropdown-item-hover-parent mcdropdown-item-hover"
                            );
                        $(this).addClass("mcdropdown-item-hover");
                    },
                    function () {
            ...
            var $current = $keylist.find(
                "li[" + settings.valueAttr + "='" + $hidden.val() + "']"
            );

            // make sure the last match is still highlighted
            $current.addClass(
                "mcdropdown-item-hover" +
                    ($current.is(".mcdropdown-item-parent") ? "-parent" : "")
            );
        ...
        function checkKeypress(e) {
            ...
            if (e.keyCode == 38) {
                ...
                compare =
                    e.keyCode == 37
                        ? ""
                        : currentNode.substring(0, currentNode.length - 1);
            ...
            if (matchesCache.length > 0) {
                ...
                compare = compare.length
                    ? compare.substring(0, compare.length - 1)
                    : "";
        ...
        function moveMatch(step) {
            ...
            if ($current.length == 0)
                $current = matchesCache.filter(
                    ".mcdropdown-item-hover, .mcdropdown-item-hover-parent"
                );
        ...
        function findMatches($lis, compare) {
            ...
            $lis.each(function () {
                ...
                if (
                    label.substring(0, compare.length).toLowerCase() == compare
                ) {
        ...
        function updateValue($li, keepTypedText) {
            ...
            var $siblings = keepTypedText
                ? matchesCache
                : $li.length == 0 || $li.hasClass("mcdropdown-item-root")
                ? $list.find("> li")
                : $li.parent().find("> li");
            ...
            if (!keepTypedText)
                typedText =
                    treePath.join(settings.delim).toLowerCase() +
                    (treePath.length > 0 ? settings.delim : "");
            ...
            setSelection(
                $self[0],
                typedText.length,
                currentNode.length + typedText.length
            );

            // remove any currently selected items
            $siblings
                .filter(".mcdropdown-item-hover,.mcdropdown-item-hover-parent")
                .removeClass(
                    "mcdropdown-item-hover mcdropdown-item-hover-parent"
                );
            // add the hover class
            $li.addClass(
                "mcdropdown-item-hover" +
                    ($li.is(".mcdropdown-item-parent") ? "-parent" : "")
            );
