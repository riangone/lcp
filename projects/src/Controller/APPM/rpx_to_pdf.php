...
class rpx_to_pdf
{
    ...
    public function to_pdf()
    {
        ...
        $tcpdf_file = str_replace("APPM", "Component", $this->file_path) . "/" . 'tcpdf/tcpdf/tcpdf.php';
        ...
        // $ts = $date -> getTimestamp().rand(0, 9999);
        $ts = date('YmdHi') . "_" . rand(0, 9999);
        //20170531 YIN UPD E
        ...
        foreach ($files as $key => $value) {
            ...
            if (strpos($value, "output_") !== false) {
                ...
                //20170531 YIN INS S
                $tmp_ts = substr($tmp_ts, 0, 12);
                $its = substr($ts, 0, 12);
                $its = strtotime($its);
                //20170531 YIN INS E
                $tmp_ts = strtotime($tmp_ts);
                //20170531 YIN UPD S
                // if ($ts - $tmp_ts >= $this -> OVER_TIME)
                if ($its - $tmp_ts >= $this->OVER_TIME)
                //20170531 YIN UPD E
                {
