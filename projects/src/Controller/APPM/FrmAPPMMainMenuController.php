...
// App::uses('FrmAPPMMainMenu', 'Model/APPM');
namespace App\Controller\APPM;

use App\Controller\AppController;
use App\Model\APPM\FrmAPPMMainMenu;

//*******************************************
// * sample controller
//*******************************************
class FrmAPPMMainMenuController extends AppController
{
    // cakephp用の設定
...
// public $autoRender = false;
    ...
    public $FrmAPPMMainMenu;

    //　デフォルトで最初に実行される機能
    public function index()
    {
        // レイアウトファイルの指定(app/View/Layouts/smpkame_layout.ctpを参照)
        // $this->layout = 'FrmAPPMMainMenu_layout';
        $this->set('title_for_layout', 'APPM広アプシステム：メニュー画面');
        $this->render('index', 'FrmAPPMMainMenu_layout');
    }

    public function menuAPPM()
    {
        // 変数の初期化
        $result = array();

        // 呼出クラスのインスタンス作成
        $this->FrmAPPMMainMenu = new FrmAPPMMainMenu();
        $STYLE_ID = "001";
        $PATTERN_ID = "001";
        $SYS_KB = "13";
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            // 処理の実行
            $menudata = $this->FrmAPPMMainMenu->menu($STYLE_ID, $PATTERN_ID, $SYS_KB);
            $data = $menudata["data"];
