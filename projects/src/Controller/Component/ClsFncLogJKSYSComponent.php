...
class ClsFncLogJKSYSComponent extends Component
{
    public $objSw = '';
    ...
    public $GS_OUTPUTLOG = array(
        "strState" => '', //OK:正常終了 NG:異常終了
        "strID" => '', //処理名
        "strNaiyou" => '', //処理内容
        "strStartDate" => '', //処理開始システム日付
        "strEndDate" => '', //処理終了システム日付
        "strDataNM" => '', //作成CSVデータ名
        "strErrNO" => '', //ｴﾗｰ注文書NO
        "strErrFlg" => '', //ｴﾗｰ件数ﾌﾗｸﾞ
        "lngCount" => '', //作成件数
    ...
    public function fncOutChk($strFileNm)
    {
        try {
            ...
            if (trim($strFileNm) == "") {
                ...
                $intHitNum = strripos($strFileNm, '/');
                if (!$intHitNum) {
                    $intHitNum = strlen($strFileNm);
                }
                $strPath = substr($strFileNm, 0, $intHitNum);

                if (!$this->ClsComFncJKSYS->FncFileExists($strPath)) {
                    return "W0015";
                }
