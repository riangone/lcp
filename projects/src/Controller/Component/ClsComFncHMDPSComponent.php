...
class ClsComFncHMDPSComponent extends Component
{
    //メンバー変数宣言
    public $xml = "";
    public $xml_path = "";
    ...
    {
        try {
            $this->ClsComFnc = new ClsComFncHMDPS();
...
//
    ...
    public function FncGetTorihikisakiMstValue($strCode = "", $all = False)
    {
        ...
        try {
            ...
            if ($objDR['row'] > 0) {
                $result['intRtnCD'] = 1;
                $result['strTorihikiNM'] = $this->FncNv($objDR["data"][0]["ATO_DTRPTBNM"]);
            } else {
                $result['intRtnCD'] = -1;
            }
            if ($all) {
