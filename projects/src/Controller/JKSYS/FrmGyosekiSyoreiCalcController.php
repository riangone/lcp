...
use App\Model\JKSYS\FrmGyosekiSyoreiCalc;
use App\Controller\JKSYS\BsyoreiInfoCsvController;

//*******************************************
// * sample controller
//*******************************************
class FrmGyosekiSyoreiCalcController extends AppController
{
    // cakephp用の設定
    ...
    private $prvArgNM = "営業業績奨励金計算処理";
    public $frmGyosekiSyoreiCalc;

    //　デフォルトで最初に実行される機能
    public function index()
    {
        $this->render('index', 'FrmGyosekiSyoreiCalc_layout');
    }

    //フォームロード
    public function frmGyosekiSyoreiCalcLoad()
    {
        $result = array(
            'result' => FALSE,
            'error' => ''
        ...
        try {
            $this->frmGyosekiSyoreiCalc = new FrmGyosekiSyoreiCalc();
            $tblCTL = $this->frmGyosekiSyoreiCalc->procGetJinjiCtrlMst_YM();
            if (!$tblCTL['result']) {
    ...
    public function btnActionClick()
    {
        ...
        try {
            if (isset($_POST['data'])) {
                ...
                //トランザクション開始
                $this->frmGyosekiSyoreiCalc->Do_transaction();
                $blnTran = TRUE;
                //1.業績奨励金データの前準備
                $result_del = $this->procPreGyosekiSyoureiKinData($dtpYM);
                ...
                //3.業績奨励金係数データの登録(INSERT) *一部あとでUPDATE
                $result_ins = $this->frmGyosekiSyoreiCalc->procInsertGyosekiSyoreiKeisuData($dtpYM);
                if (!$result_ins['result']) {
                    throw new \Exception($result_ins['data']);
                }
                //4.業績奨励金係数データの更新(UPDATE)
                $result_upd = $this->frmGyosekiSyoreiCalc->procUpdateGyosekiSyoreiKeisuData($dtpYM);
                if (!$result_upd['result']) {
                    throw new \Exception($result_upd['data']);
                }
                //5.業績奨励金データの登録(INSERT)
                $result_ins = $this->frmGyosekiSyoreiCalc->procInsertGyosekiSyoureiKinHdrData($dtpYM);
                if (!$result_ins['result']) {
                    throw new \Exception($result_ins['data']);
                }
                //6.奨励金マスタの更新(PHP中此表相应的字段没用到，不更新)

                //コミット
                $this->frmGyosekiSyoreiCalc->Do_commit();
                $blnTran = FALSE;
                //7.CSV出力
                // include_once dirname(__DIR__) . "/JKSYS/BsyoreiInfoCsvController.php";
                $bsyoreiInfoCsv = new BsyoreiInfoCsvController();
                $bsyoreiInfoCsv->intSyourei_Kbn = "1";
            ...
            if ($blnTran) {
                $this->frmGyosekiSyoreiCalc->Do_rollback();
            }
        }
        $this->fncReturn($result);
    }
    ...
    public function procCheckData($dtpYM)
    {
        ...
        try {
            ...
            for ($intIdx = 0; $intIdx < count($strDataNM); $intIdx++) {
                ...
                $result_check = $this->frmGyosekiSyoreiCalc->procCheckDataLogic($intIdx, $strDataTblNM[$intIdx], $dtpYM);
                if (!$result_check["result"]) {
    ...
    //業績奨励金データの削除
    public function procPreGyosekiSyoureiKinData($dtpYM)
    {
        $result = array(
            'result' => FALSE,
            'error' => ''
        ...
        try {
            //1.JKGYOSEKISYOREI
            $result_del = $this->frmGyosekiSyoreiCalc->proDeleteJKGYOSEKISYOREI($dtpYM);
            if (!$result_del['result']) {
                throw new \Exception($result_del['data']);
            }
            //2.JKGYOSEKISYOREIKEISU
            $result_del = $this->frmGyosekiSyoreiCalc->proDeleteJKGYOSEKISYOREIKEISU($dtpYM);
            if (!$result_del['result']) {
                throw new \Exception($result_del['data']);
            }
            $result['result'] = true;
