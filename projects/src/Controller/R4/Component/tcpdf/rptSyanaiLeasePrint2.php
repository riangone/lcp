 ...
 *
 */
function rptSyanaiLeasePrint2($key, $data)
{
    $return = array();
    $return["val"] = true;
    $strToday = "";
    switch ($key) {
        ...
        case "SYUTOKU_KIN_Total":
            $data['SYUTOKU_KIN_Total'] = (int) $data['KIKAI_KIN'] + (int) $data['KOUGU_KIN'];
            if ($data['SYUTOKU_KIN_Total'] == "") {
                $data['SYUTOKU_KIN_Total'] = "0";
            }
            break;
        case "LEASE_RYO_Total":
            $data['LEASE_RYO_Total'] = (int) $data['KIKAI_REASE_RYO'] + (int) $data['KOUGU_REASE_RYO'];
