...
// * sample controller
//*******************************************
class FrmHDKAIKEIMainMenuController extends AppController
{
    // cakephp用の設定
...
// public $autoRender = false;
    ...
    public function index()
    {
        ...
        // $this->layout = 'FrmHDKAIKEIMainMenu_layout';

        $this->set('title_for_layout', '伝票集計システム：メニュー画面');
        $this->render('index', 'FrmHDKAIKEIMainMenu_layout');
    }

    public function menuHDKAIKEI()
    {
        // 変数の初期化
        $PATTERN_ID = "";
        $result = array();
        ...
        $roledata = $this->FrmHDKAIKEIMainMenu->getmenulist($login_user, HDKAIKEIController::SYS_KB);
        $role = $roledata["data"];
        if ($role) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            // 処理の実行
            $menudata = $this->FrmHDKAIKEIMainMenu->menu(HDKAIKEIController::STYLE_ID, $PATTERN_ID, HDKAIKEIController::SYS_KB);
    ...
    public function fncChkUserAuthority()
    {
        ...
        try {
            ...
            $Session = $this->request->getSession();
            $login_user = $Session->read('login_user');
            $data = $this->FrmHDKAIKEIMainMenu->FncChkUserAuthority($login_user, $postData['PRO_ID'], HDKAIKEIController::SYS_KB, HDKAIKEIController::STYLE_ID);
            if (!$data['result']) {
