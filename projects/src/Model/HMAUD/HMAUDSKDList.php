...
// * 処理説明	：機能名のデータ処理クラス
//*************************************
class HMAUDSKDList extends ClsComDb
{
    ...
    public function fncSelectMSTdataSQL()
    {
        ...
        $strSql .= " WHERE H.ENABLED = '1' " . "\r\n";
        $strSql .= " ORDER BY TO_NUMBER(H.SEQ) ASC  " . "\r\n";
        return $strSql;
    }

    public function getRiyouAmList($yearMonth, $ap)
    {
        $strSql = $this->getRiyouAmListSQL($yearMonth, $ap);
        return parent::select($strSql);
    }

    public function getRiyouAmListSQL($yearMonth, $ap)
    {
        $strSql = "";
        $strSql .= " SELECT  " . "\r\n";
        $strSql .= " H.SYAIN_NO || TO_CHAR(S.PLAN_DT,'YYYY-MM-DD') AS SYAINDT " . "\r\n";
        $strSql .= " ,S.ENABLED " . "\r\n";
        $strSql .= " FROM HMAUD_MST_AUDITOR H " . "\r\n";
        $strSql .= " LEFT JOIN HMAUD_AUDITOR_SCHEDULE S  " . "\r\n";
        $strSql .= " ON S.SYAIN_NO = H.SYAIN_NO" . "\r\n";
        $strSql .= " WHERE TO_CHAR(S.PLAN_DT,'YYYYMM') = '@YM' " . "\r\n";
        $strSql .= " AND S.AMPM = '@AMPM'" . "\r\n";
        $strSql .= " AND H.ENABLED = '1'" . "\r\n";
        $strSql .= " ORDER BY TO_NUMBER(H.SEQ) ASC " . "\r\n";

        $strSql = str_replace("@YM", $yearMonth, $strSql);
        $strSql = str_replace("@AMPM", $ap, $strSql);
